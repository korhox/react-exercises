...
import { deleteAPI } from '../util/fetchers';
import { modalContext } from '../util/context';

const IssueComponent: FC<{ id: string; content: string }> = ({ id, content }) => {
    const { mutate } = useSWRConfig();
    const removeIssue = async () => {
        mutate('/boards/', await deleteAPI(`/issues/${id ?? ''}`, `/boards?_embed=issues`));
    };
    const modal = useContext(modalContext);
    return (
        <>
            <div className="group relative mb-2 flex w-full rounded bg-white/70 p-2 text-left shadow transition-all duration-200 ease-in-out hover:bg-white hover:shadow-lg" id={'issue-' + id}>
                <div>
                ...
                <div className="absolute top-0 bottom-0 right-0 flex justify-end pr-1 align-middle opacity-0 transition-opacity group-hover:opacity-100">
                    ...
                    <button onClick={() => modal.setContent(<p>Foo</p>)} className="p-1 opacity-60 contrast-75 drop-shadow-sm transition-all duration-200 ease-in-out hover:scale-125 hover:text-orange-500 hover:opacity-100">
