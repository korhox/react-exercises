...
const BoardComponent: FC<{ id: string; title: string; issues: Issue[]; order: number }> = ({ id, title, issues, order }) => {
    ...
    return (
        <div draggable onDragStart={(e) => e.dataTransfer.setData('text/plain', `board-${id}`)} data-order={order} ref={board} id={'board-' + id} onDrop={moveIssue} onDragOver={(e) => e.preventDefault()} className="m-3 w-[300px] rounded-lg bg-white/60 p-2 pb-0 shadow-md backdrop-blur">
